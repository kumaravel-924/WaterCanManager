...
const db = {
  ...
  async addTransaction(p) {
    ...
    if (error) {
      // If the column 'paid_today' is missing in the DB, try saving without it
      if (error.message && error.message.includes('paid_today')) {
        toast('⚠️ Warning: Payment not saved. Database column missing.');
        const { paid_today, ...fallback_p } = p;
        const { data: d2, error: e2 } = await _supabase.from('transactions').insert([fallback_p]).select().single();
  ...
  async getAllSalesSummary() {
    ...
    const byDate = {};
    rows.forEach(t => {
      if (!byDate[t.date]) byDate[t.date] = { date: t.date, total_br: 0, total_bc: 0, total_paid: 0, count: 0 };
      byDate[t.date].total_br += n(t.balance_rupees);
      byDate[t.date].total_bc += n(t.balance_can);
      byDate[t.date].total_paid += n(t.paid_today);
      byDate[t.date].count++;
    });
...
function modalPrompt(title, sub, placeholder, secret, onConfirm) {
  const ov = document.createElement('div'); ov.className = 'modal-overlay';
  ov.innerHTML = `
    <div class="modal-sheet">
      <div class="modal-handle"></div>
      <div class="confirm-ico">🛡️</div>
      <div class="modal-title" style="text-align:center;margin-bottom:8px;">${title}</div>
      <div style="text-align:center;color:var(--text3);font-size:14px;margin-bottom:18px;">${sub}</div>
      <div class="form-group">
        <label class="form-label">Secret Code</label>
        <input type="password" id="p-input" class="form-input" placeholder="${placeholder}">
      </div>
      <div class="btn-group">
        <button class="btn btn-outline" id="p-cancel">Cancel</button>
        <button class="btn btn-danger" id="p-confirm">Confirm Wipe</button>
      </div>
    </div>
  `;
  document.body.appendChild(ov);
  ov.querySelector('#p-cancel').onclick = () => ov.remove();
  ov.querySelector('#p-confirm').onclick = async () => {
    const val = ov.querySelector('#p-input').value;
    if (val !== secret) return toast('❌ Invalid Secret Code');
    const b = ov.querySelector('#p-confirm'); b.disabled = true; b.textContent = 'Processing…';
    await onConfirm(); ov.remove();
  };
}
...
function pgSettings() {
  ...
  document.getElementById('wipe-btn').onclick = () => modalPrompt(
    'Identity Verification',
    'Enter the Secret Code to delete all data.',
    'Enter code here...',
    'prasanth',
    async () => {
      if (useLocal) {
  ...
  el.querySelectorAll('.cust-card').forEach(card => {
    card.onclick = e => {
      if (e.target.closest('.q-del')) {
        ...
        const cname = fil.find(cx => cx.id === cid)?.name || 'Customer';
        modalPrompt(
          `Delete ${cname}?`,
          'Enter secret code to delete this customer and their entire history.',
          'Enter code...',
          'prasanth',
          async () => { await db.deleteCustomer(cid); toast('🗑️ Deleted'); pgHome(); }
        );
        return;
      }
...
function modalAddCustomer() {
  ...
  ov.innerHTML = `
    <div class="modal-sheet">
      ...
      <div class="form-group"><label class="form-label">Balance (₹)</label><input type="number" id="m-amt" class="form-input" value="0"></div>
  ...
  try {
      ...
      <div class="page">
        <div class="detail-hero">
          ...
          <div class="hero-badges">
            ...
            <div class="hero-badge">💸 ₹${n(c.balance_rupees).toFixed(0)} Balance</div>
        ...
        <div class="card" style="padding:16px;margin-bottom:14px;">
          ...
          <div class="sum-grid">
            ...
            <div class="form-group"><label class="form-label">Pay Today (₹)</label><input type="number" id="u-pay" class="form-input" placeholder="0"></div>
          </div>
          <div class="sum-grid">
            <div class="form-group"><label class="form-label">Balance (₹)</label><input type="number" id="u-amt" class="form-input" value="${c.balance_rupees}"></div>
    ...
    document.getElementById('save-btn').onclick = async () => {
      ...
      try {
        ...
        const paidToday = n(document.getElementById('u-pay').value);
        
        // If they paid today, we could automatically subtract from balance, 
        // but let's keep it manual as per original behavior.
    ...
    document.getElementById('del-btn').onclick = () => modalPrompt(
      `Delete ${c.name}?`,
      'Enter secret code to permanently remove this customer and all history.',
      'Enter code...',
      'prasanth',
  ...
  return `
    <div class="tx-item" data-id="${t.id}">
      ...
      <div class="tx-badges">
        ...
        <span class="badge b-amt">₹${n(t.balance_rupees).toFixed(0)} bal</span>
        ${n(t.paid_today) > 0 ? `<span class="badge b-amt" style="background:var(--green);color:#fff;">💰 Paid ₹${n(t.paid_today).toFixed(0)}</span>` : ''}
  ...
  try {
    ...
    main.innerHTML = `
      <div class="page">
        <div class="detail-hero">
          <div class="hero-name">${c.name}</div>
          <div class="hero-sub">🪣 ${n(c.balance_can)} cans · ₹${n(c.balance_rupees).toFixed(0)} balance · AP ₹${n(c.advance_payment).toFixed(0)}</div>
        </div>
        <div class="sec-head">
...
async function pgToday() {
  ...
  try {
    ...
    const customers = await db.getCustomers(); // Fetch fresh customer list for true balance
    const totalBusinessBalance = customers.reduce((s, c) => s + n(c.balance_rupees), 0);
    const totalBC_daily = sales.reduce((s, t) => s + n(t.balance_can), 0);
    const totalPAID_daily = sales.reduce((s, t) => s + n(t.paid_today), 0);
    const totalAP_daily = sales.reduce((s, t) => s + n(t.advance_payment), 0);
    ...
    main.innerHTML = `
      <div class="page">
        ...
        <div class="sum-grid">
          <div class="sum-card"><div class="sum-label">💰 Paid Today</div><div class="sum-val green">₹${totalPAID_daily.toFixed(0)}</div></div>
          <div class="sum-card"><div class="sum-label">💸 Balance</div><div class="sum-val amber">₹${totalBusinessBalance.toFixed(0)}</div></div>
          <div class="sum-card"><div class="sum-label">🪣 Cans Out</div><div class="sum-val blue">${totalBC_daily}</div></div>
          <div class="sum-card"><div class="sum-label">👥 Customers</div><div class="sum-val">${sales.length}</div></div>
        </div>
...
async function pgAllSales() {
  ...
  try {
    ...
    const customers = await db.getCustomers(); // Fetch fresh customer list
    const totalBusinessBalance = customers.reduce((s, c) => s + n(c.balance_rupees), 0);
    const totalPaid = summary.reduce((s, d) => s + d.total_paid || 0, 0);
    ...
    main.innerHTML = `
      <div class="page">
        <div class="stats-strip">
          <div class="stat-chip"><div class="stat-lbl">Total Paid</div><div class="stat-val" style="color:var(--green)">₹${totalPaid.toFixed(0)}</div></div>
          <div class="stat-chip"><div class="stat-lbl">Total Balance</div><div class="stat-val" style="color:var(--amber)">₹${totalBusinessBalance.toFixed(0)}</div></div>
        ...
        : `<div class="card">${summary.map(d => `
            <div class="cust-card" data-date="${d.date}">
              ...
              <div class="cust-right">
                <span class="badge b-amt" style="background:var(--green);color:#fff;">Paid ₹${(d.total_paid || 0).toFixed(0)}</span>
                <span class="badge b-amt" style="margin-top:4px;">Bal ₹${d.total_br.toFixed(0)}</span>
              </div>
            </div>`).join('')}
          </div>`
      }
...
CREATE TABLE transactions (
  ...
  balance_rupees NUMERIC(10,2) DEFAULT 0,
  advance_payment NUMERIC(10,2) DEFAULT 0,
  paid_today NUMERIC(10,2) DEFAULT 0,
...
function renderShell() {
  ...
  document.getElementById('app').innerHTML = `
    ...
    <nav id="bottom-nav">
      ...
      <button class="nav-btn ${S.page === 'today-sales' ? 'active' : ''}" onclick="S.navDate=null; go('today-sales')">
